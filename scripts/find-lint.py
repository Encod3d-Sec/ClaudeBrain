...
def section_nonempty(text, header_re):
    ...
    Fenced code blocks are tracked so a shell comment inside one (```bash
    followed by `# do the thing`) is not mistaken for a markdown heading. That
    bug truncated a PoC body to the fence line alone and failed findings whose
    reproduction opened with a commented command - a very common shape.
    """
    ...
    for i, ln in enumerate(lines):
        if FENCE_RE.match(ln):
            ...
            continue
        if in_fence or not re.match(header_re, ln.strip(), re.I):
            continue
        body, body_fence = [], False
        for nxt in lines[i + 1:]:
            if FENCE_RE.match(nxt):
                body_fence = not body_fence
            elif not body_fence and re.match(r"^#+\s", nxt):
                break
            body.append(nxt)
        txt = "\n".join(body).strip()
