...
# tmux parses '.' as its pane separator and ':' as its window separator, so a SESSION name carrying
# either is silently rewritten on create and then unaddressable by the name we passed
# (site.example -> site_example). '/' survives creation but breaks capture.sh's default target.
# Sanitize once, HERE, so every caller (recon-web.sh, the hooks, capture.sh recon) agrees on one name.
