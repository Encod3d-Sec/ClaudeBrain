...
type: technique
tags: [enumeration, reference-import, web]
phase: recon
...
Technical reference for **Web Attack Surface** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Web attack surface enumeration maps all externally accessible entry points for a target organization: subdomains, IP ranges, exposed services, web technologies, and third-party integrations. Subdomain enumeration via certificate transparency logs (crt.sh), passive DNS databases (SecurityTrails, VirusTotal), and brute-forcing reveals hosts that may host vulnerable or forgotten services. Subdomain takeover vulnerabilities arise when a subdomain's CNAME points to an unclaimed resource in a third-party service, allowing an attacker to claim the resource and serve content from the victim's subdomain.
