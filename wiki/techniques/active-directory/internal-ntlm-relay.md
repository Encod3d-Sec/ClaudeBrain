...
NTLM relay attacks intercept a NetNTLM authentication challenge and response from a victim machine and forward it to a target service, authenticating as the victim without ever cracking the hash. An attacker poisons name resolution (via LLMNR, NBT-NS, or mDNS using Responder) to redirect authentication to `ntlmrelayx`, which relays the credential to LDAP, SMB, HTTP, or MSSQL targets that accept NTLM. Successful relay against LDAP can dump secrets, add computers, or modify ACLs; relay to SMB can create remote shares or execute commands if SMB signing is not enforced.
