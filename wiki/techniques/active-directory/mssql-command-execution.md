...
Technical reference for **MSSQL - Command Execution** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

MSSQL provides multiple mechanisms for executing OS commands from within a SQL session, most prominently `xp_cmdshell`, which passes commands to `cmd.exe` and returns the output as rows. When `xp_cmdshell` is disabled, attackers use alternatives such as CLR assemblies (loading a .NET DLL that executes managed code), OLE Automation procedures (`sp_OACreate`), SQL Agent jobs, or external scripts (Python, R) to achieve equivalent OS command execution. All of these require `sysadmin` or equivalent SQL Server privileges, making privilege escalation within SQL Server a prerequisite for this vector.
