...
Technical reference for **Windows - Privilege Escalation** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Windows local privilege escalation leverages misconfigurations in service binaries (unquoted service paths, writable service executables), token privileges (SeImpersonatePrivilege, SeDebugPrivilege), weak registry ACLs on service keys, DLL hijacking in privileged application directories, and unpatched kernel vulnerabilities to escalate from a standard user to SYSTEM. Tools like WinPEAS, PowerUp, and PrivescCheck automate enumeration of these conditions, reporting weak permissions, stored credentials, scheduled tasks, and kernel exploit candidates. The exploited property varies: unquoted paths abuse Windows path resolution order, SeImpersonatePrivilege enables token impersonation via named pipe tricks (Potato attacks), and DLL hijacking places a malicious DLL in a location searched before the legitimate one.
