...
title: Mimikatz
type: technique
tags: [active-directory, credentials, reference-import, windows]
...
Technical reference for **Mimikatz** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Mimikatz is a Windows credential extraction tool that reads plaintext passwords, NTLM hashes, Kerberos tickets, and cryptographic keys directly from LSASS process memory by opening a handle to the process with `SeDebugPrivilege`. It enables attacks including Pass-the-Hash, Pass-the-Ticket, Golden Ticket creation, DCSync, skeleton key injection, and DPAPI decryption, covering nearly every Windows credential-related attack primitive. Mimikatz requires local administrator rights at minimum and SYSTEM or `SeDebugPrivilege` for LSASS memory access, making it a post-exploitation tool rather than an initial access one.
...
[diagram: Mimikatz credential data in LSASS memory](http://adsecurity.org/wp-content/uploads/2014/11/Delpy-CredentialDataChart.png)
...
- [[mimikatz]]
- [[radare2]]
- Also uses (no dedicated page yet): PowerSploit
