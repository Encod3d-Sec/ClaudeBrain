...
Technical reference for **Windows - Using credentials** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Once credentials are captured (NTLM hash, plaintext password, or Kerberos ticket), they are used to authenticate to remote services using tools like Impacket (`psexec.py`, `wmiexec.py`, `smbexec.py`), NetExec, or native Windows RDP and PowerShell remoting. Pass-the-Hash uses an NTLM hash directly with these tools without cracking, while Pass-the-Ticket injects a captured Kerberos ticket for Kerberos-authenticated services. The choice of lateral movement method (PSExec, WMI, SMB, RDP, WinRM) depends on which services are accessible on the target, what ports the firewall allows, and which execution method leaves the smallest forensic footprint.
