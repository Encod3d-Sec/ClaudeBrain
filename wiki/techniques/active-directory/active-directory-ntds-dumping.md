...
type: technique
tags: [active-directory, credentials, ntds, reference-import, windows]
...
The `NTDS.dit` database on a Domain Controller stores all Active Directory objects including NTLM password hashes for every domain account, encrypted with keys derived from the SYSTEM registry hive. Attackers with Domain Admin or DCSync rights either extract the file directly (using VSS shadow copies or `ntdsutil`) or replicate it remotely using the DCSync technique via Mimikatz `lsadump::dcsync`, which abuses the legitimate AD replication protocol (`MS-DRSR`). The resulting hashes can be cracked offline or used directly in Pass-the-Hash attacks to authenticate as any domain account.
