...
type: technique
tags: [exploitation, reference-import, reverse-shell, web-shell]
phase: exploitation
...
Technical reference for **Reverse Shell Cheat Sheet** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Reverse shells establish an outbound connection from the target to the attacker's listener, bypassing inbound firewall rules that would block a bind shell. Shells can be spawned using built-in interpreters present on the target (bash, Python, Perl, PHP, PowerShell, nc) or via process injection and in-memory shellcode execution. The choice of reverse shell language and method depends on what interpreters and network protocols are available on the target and what the target firewall permits outbound.
