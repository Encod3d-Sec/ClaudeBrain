...
type: technique
tags: [enumeration, network, reference-import]
phase: enumeration
...
Technical reference for **Network Discovery** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Network discovery maps the internal network topology by identifying active hosts, open ports, running services, and network infrastructure (routers, gateways, DNS servers) from a compromised foothold. Attackers use tools like `nmap`, `netdiscover`, and `arp-scan` for host enumeration, combined with MAC address OUI lookups to identify device manufacturers and infer network topology. Changing a network interface's MAC address before scanning helps evade network access control (NAC) systems that restrict access based on registered MAC addresses.
