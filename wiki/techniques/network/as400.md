...
title: AS400
type: technique
tags: [as400, exploitation, network, reference-import]
phase: exploitation
...
IBM AS400 (IBM i) systems are accessed over TCP ports 449 (DDM), 8470 (Telnet), and 9470 (5250 emulation); attackers use tools like `tn5250` or Metasploit modules to enumerate user profiles, spray common default passwords, and execute commands via the CL (Control Language) interface. The integrated DB2 database is accessible via SQL and can be queried for sensitive application data once authentication is obtained. Privilege escalation targets the `*ALLOBJ` and `*SECOFR` special authorities, which grant unrestricted system access.
...
[diagram: AS/400 attack mind map](https://web.archive.org/web/20140830222720if_/http://www.toolswatch.org/wp-content/uploads/2013/02/AS400.jpg)
