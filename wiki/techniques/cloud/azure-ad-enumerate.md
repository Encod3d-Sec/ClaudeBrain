...
type: technique
tags: [active-directory, azure, cloud, enumeration, reference-import]
phase: enumeration
...
Technical reference for **Azure AD - Enumerate** collected from InternalAllTheThings during an internal/cloud assessment ingest.

## How it works

Azure AD enumeration collects tenant structure, user accounts, groups, roles, service principals, application registrations, and role assignments using authenticated REST API calls to the Microsoft Graph API. Tools like ROADrecon, AzureHound, and the Azure CLI automate this by querying `graph.microsoft.com` endpoints and building a graph of all relationships, which BloodHound can then analyze for privilege escalation paths. Any valid Azure AD user account, including guest accounts, can perform significant enumeration because read access to the directory is granted by default.
