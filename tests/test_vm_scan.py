...
def test_session_name_is_tmux_sanitized():
    # tmux parses '.' as a pane separator and rewrites the session name on create, leaving the
    # original unaddressable. An engagement path carries both '/' and '.', so sanitize both.
    out = dry("prog1/app.example.com", "t", "echo hi")
    assert "prog1-app-example-com" in out
